...
function getAllColors() {
    ...
    function collectColors(item) {
        ...
        if (item.typename === "GroupItem") {
            ...
            $.writeln("item.pathItems.length: ", item.pathItems.length)
            $.writeln("item.pathItems[0].fillColor: ", item.pathItems[0].fillColor)
            addColor(item.pathItems[0].fillColor, item);
        } else {
            if (item.filled && item.fillColor) {
                addColor(item.fillColor, item);
            }
        }
    }

    function addColor(color, item) {
        if (color.typename === "RGBColor") {
            ...
            item.selected = true;
            // $.writeln("Gray Item: " + item);
            // $.writeln("Gray Item Color: " + color.gray);
